...
function DeathKnight:Blood()
	...
	if buff[BL.DancingRuneWeaponBuff].up and buff[BL.BoneShield].remains < 3 then
		return BL.Marrowrend
	end
	
	if buff[BL.BoneShield].remains < 3 then
		return BL.DeathsCaress
	end

	if runicPower > 35 and (runicPower > 80 or healthPercent < 65) then
		return BL.DeathStrike
	end

	if cooldown[BL.BloodBoil].ready and cooldown[BL.BloodBoil].charges > 1 then
		return BL.BloodBoil
	end

	if buff[BL.BoneShield].count <= 6 then
		return BL.Marrowrend
	end
	...
	if cooldown[BL.BloodBoil].ready then
		return BL.BloodBoil
	end
...
function DeathKnight:BloodCooldowns()
	...
	if healthPercent < 55 and cooldown[BL.AntiMagicShell].ready then
		return BL.AntiMagicShell
	end
	
	if healthPercent < 50 and cooldown[BL.VampiricBlood].ready then
		return BL.VampiricBlood
	end
	
	if healthPercent < 35 and cooldown[BL.IceboundFortitude].ready then
